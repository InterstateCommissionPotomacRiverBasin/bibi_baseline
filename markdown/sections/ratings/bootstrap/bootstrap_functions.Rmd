...
nest_df <- function(rating.df, su.df, su.res) {
  ...
  test <- rating.df %>% 
    ...
    select(!!su.res, rating, spatial_period) %>% 
    group_by(!!su.res, spatial_period) %>% 
    nest() %>% 
...
stat_ratings <- function(pct.ratings.list, boot.sample) {
  purrr::map(1:4, function(i) {
    purrr::map(pct.ratings.list, i) %>% 
      unlist() %>% 
      psych::describe() %>% 
      dplyr::rename(percentage = mean)
  }) %>% 
    bind_rows() %>% 
    mutate(rating = c("Acceptable", "Fair", "Degraded", "Insufficient"),
           rating = factor(rating, c("Acceptable", "Fair", "Degraded", "Insufficient")),
