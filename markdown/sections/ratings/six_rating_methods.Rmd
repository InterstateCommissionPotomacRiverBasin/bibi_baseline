...
`mean_cell_rating()` makes it possible to calculate the mean Chessie BIBI score for each spatial unit cell, assign the appropriate Chessie BIBI rating based on the mean score, and summarize the results to represent the proportion of each rating represented in the Chesapeake Bay basin.
```{r}
...
loop_cell_rating <- function(bibi.df, su.df, unit.col, su.vec, job) {
  ...
  purrr::map_df(su.vec, function(su.i) {
    ...
    cell.df <- aggregate_su(su.df, !!su.i, !!unit.col)
    #--------------------------------------------------------------------------
    bibi.cell <- bibi.df %>% 
      full_join(cell.df, by = rlang::quo_name(su.i)) %>% 
      complete(spatial_period, nesting(!!su.i, !!unit.col, total)) %>% 
      filter(!is.na(spatial_period))
    #--------------------------------------------------------------------------
    if (job == "mean_rating") {
      final.df <- mean_cell_rating(bibi.cell, !!su.i, !!unit.col)
    } 
    
    if (job == "weighted_rating") {
      final.df <- weighted_cell_rating(bibi.cell, !!su.i, !!unit.col)
    } 
    #--------------------------------------------------------------------------
    final.df <- final.df %>% 
